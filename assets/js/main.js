...
(function () {
  ...
  /* 모바일에서 타일 오버레이(글) 배치를 통일: Posted를 아래에 고정 */
@media screen and (max-width: 736px) {

	/* 타일 링크 영역을 기준점으로 */
	.tiles article > a {
	  justify-content: flex-start; /* 가운데정렬로 밀리는 현상 방지 */
	...
	/* content를 화면 아래로 밀어 고정 */
	.tiles article > a .content {
	  width: 100%;
	  margin-top: auto;           /* 핵심: 남는 공간을 위로 보내서 content를 아래로 */
	  max-height: none;           /* 모바일에서 is-active로 보일 때도 안정 */
	  opacity: 1;                 /* 모바일에서 보일 때 기준 (너가 is-active 쓰면 이건 선택) */
	}
  
	/* Posted 문장만 딱 같은 위치/간격 */
	.tiles article > a .content p:last-child {
	  margin: 0;                  /* 기본 p의 2em margin 제거 */
	  font-size: 0.85em;
	  letter-spacing: 0.06em;
	}
  
	/* 너가 넣어둔 빈 <p></p> 때문에 공간 생기는 거 제거 */
	.tiles article > a .content p:empty {
	  display: none;
	}
  }